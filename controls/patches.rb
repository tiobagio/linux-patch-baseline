...
control 'available-security-updates' do
  impact 0.3
  title 'All updates available for installed packages that are security relevant'
  linux_update.sec_updates.each { |update|
    str = update['name']
    name = str.gsub(/[.].*/,'')
  ...
  only_if { linux_update.sec_updates.length > 0 }
end
